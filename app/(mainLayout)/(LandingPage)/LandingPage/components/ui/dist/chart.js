...
var ChartTooltipContent = React.forwardRef(function (_a, ref) {
    ...
    var tooltipLabel = React.useMemo(function () {
        ...
        if (labelFormatter) {
            return (React.createElement("div", { className: functions_1.cn("font-medium", labelClassName) }, labelFormatter(value, payload)));
        }
        if (!value) {
            return null;
        }
        return React.createElement("div", { className: functions_1.cn("font-medium", labelClassName) }, value);
    }, [
        label,
        labelFormatter,
        payload,
    ...
    return (React.createElement("div", { ref: ref, className: functions_1.cn("grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className) },
        ...
        React.createElement("div", { className: "grid gap-1.5" }, payload.map(function (item, index) {
            ...
            return (React.createElement("div", { key: item.dataKey, className: functions_1.cn("flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-muted-foreground", indicator === "dot" && "items-center") }, formatter && (item === null || item === void 0 ? void 0 : item.value) !== undefined && item.name ? (formatter(item.value, item.name, item, index, item.payload)) : (React.createElement(React.Fragment, null,
                (itemConfig === null || itemConfig === void 0 ? void 0 : itemConfig.icon) ? (React.createElement(itemConfig.icon, null)) : (!hideIndicator && (React.createElement("div", { className: functions_1.cn("shrink-0 rounded-[2px] border-[--color-border] bg-[--color-bg]", {
                ...
                React.createElement("div", { className: functions_1.cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center") },
...
var ChartLegendContent = React.forwardRef(function (_a, ref) {
    ...
    return (React.createElement("div", { ref: ref, className: functions_1.cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className) }, payload.map(function (item) {
        var key = "" + (nameKey || item.dataKey || "value");
        var itemConfig = getPayloadConfigFromPayload(config, item, key);
        return (React.createElement("div", { key: item.value, className: functions_1.cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground") },
